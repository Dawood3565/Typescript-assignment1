...
function saygraydient() {
  var studentlist:string[] = ["ali", "Naveed"];
  for (let index = 0; index < studentlist.length; index++) {
    console.log("hi"+index+"welcome to university");
  }
  var input:string = "Naveed"
    if (input == "Naveed") {
...
function alphabet() {
  var alphabet:string =  (<HTMLInputElement> document.getElementById("alpha")).value;
  var latter:any = /^[A-Za-z]+$/;
...
function resultfun() {
  ...
  var math:number = 30;
  var chemistry:number = 60;
  var physics:number = 40;
  var english:number = 35;
  var urdu:number = 20;
  var result:number = math+chemistry+physics+english+urdu;
  console.log(result);
